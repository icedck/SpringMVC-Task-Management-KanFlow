$(document).ready(function () {
  ...
  $(document).on("click", ".card", function (event) {
    ...
    $.when(
      ...
      $.ajax({ type: "GET", url: `/api/cards/${cardId}/attachments` }) // Lời gọi API mới
    )
  ...
  $(document).on("click", ".remove-member-btn", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        // THAY THẾ CODE CŨ BẰNG 2 DÒNG NÀY
        alert(response); // Hiển thị thông báo thành công
        location.reload(); // Tải lại trang để cập nhật toàn bộ giao diện
      },
      error: function (xhr) {
        alert("Error: " + xhr.responseText);
      },
  ...
  // === LOGIC UPLOAD/DELETE ATTACHMENT ===

  // Mở cửa sổ chọn file khi click nút "Choose File"
  ...
  $("#file-upload-input").on("change", function () {
    ...
    if (fileInput.files.length > 0) {
      ...
      $("#upload-file-button").show(); // Hiện nút Upload
    } else {
      $("#file-name-display").text("");
      $("#upload-file-button").hide(); // Ẩn nút Upload
    }
  });

  // Khi click nút "Upload"
